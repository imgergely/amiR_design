...
from db import dropbox_download_files


def app():
    with st.spinner("Initializing the app. This may take a moment..."):
    ...
    select_db_options = {"mouse": "Mouse","human": "Human"}
    ...
    if start:
        if "".__eq__(user_input_seq) or not validate(user_input_seq):
            ...
            oligos = {index:[filtered_oligos[index],filtered_oligos[index][14:21]] for index in range(0,len(filtered_oligos))} ## SEED [14:21]
            ...
            blast_results = NcbiblastnCommandline(db = "db/"+ select_db, word_size=7, dust ="no", evalue=10, num_threads=2, outfmt ="6 qseqid sseqid bitscore sseq") 
